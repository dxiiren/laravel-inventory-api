...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   ...
   and GET http://127.0.0.1:8105/api/products) to catch drift or undocumented breakage. Building a
...
- Files: `app/Http/Controllers/ProductController.php:40-48`, `app/Imports/ProductImport.php`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just start`, `just test`, `just migrate`
...
- Dev server on :8105 was {up/down} at close - re-check (`just start` / `just stop`)
- Uncommitted changes: {yes/no} - `git status`
- Migration state: {...} as of {time} - `php artisan migrate:status`
