...
`github.com/dxiiren/upload-product-laravel-excel`.
...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(products): add brand filter to product list` or
  `fix(import): skip rows with unknown product ids`. Pick the type/scope from the
...
- <bullet per meaningful change — controller/model/migration/view touched>
- <...>

## Testing

- <how it was verified: `just test` result, manual check on http://127.0.0.1:8105, etc.>
```
...
| `/pre-pr-review` | Optional self-review (Laravel/API checklist) before opening the PR            |
| `/lint-check`    | Run Pint + the test suite before the PR                                       |
