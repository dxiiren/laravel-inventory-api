...
4. **Guardrails & locked decisions** - may it `git commit` / `push` / stage? May it open a PR /
   comment on GitHub, or touch any external service? Any destructive ops (e.g. `just fresh` wipes
   the local db)? What decisions are **locked** (do-not-relitigate)? Any **environment bootstrap**
   (`just bootstrap`, the dev server, a sanity gate like `just test`)?
