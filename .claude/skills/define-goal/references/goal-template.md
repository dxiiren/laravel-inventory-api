...
- {Step to bring the world up + how to check it's up + what to do if it's down. e.g. `just
  bootstrap` if `vendor/` is missing, the dev server (`just start` on :8105), a sanity gate
  (`just test`). Reference the exact command.}
...
- **Real (may mark `BLOCKED`):** {code facts - a 404 route, a missing class import at
  `file:line`, a migration that cannot run on sqlite; or a hard infra failure you tried once to fix
  and couldn't.}
- **NOT a blocker (keep working):** "it's late", "it's complex", "needs a focused session", missing
  fixture data (-> create it via a factory), a dev server that died (-> `just start`, re-poll,
  continue).
...
grep/glob/`php artisan route:list`/test-run output.} Append one row per item found; set each to
`TODO`.
...
```markdown
# GOAL - Backfill feature tests for the three product endpoints
...
Write feature tests so `products.index` (search filter + pagination), `products.store`
(DTO validation), and `products.import` (file validation + job dispatch) each have passing
coverage. Success = `just test` green with the three new test classes present and asserting
real behavior.

## Locked decisions (do not re-litigate)

- Test behavior, not implementation - assert response content + database state, not internals.
- Use model factories (`ProductFactory`) - never hand-inserted rows; fake the queue
  (`Queue::fake`) for import dispatch assertions.

## Environment bootstrap

- `just bootstrap` if `vendor/` is missing. No dev server needed - feature tests boot the app
  themselves. Sanity gate: `just test` should already pass before you start.

## Hard prohibitions / guardrails

- NEVER git commit/push/stage - leave changes in the working tree for review.
- No GitHub PR/issue posts.
- Touch only `tests/Feature/` (+ a factory tweak if a state is missing).

## What counts as a REAL blocker

- Real: an endpoint depends on a service with no test double and no reasonable fake -
  cite the `file:line`.
- Not a blocker: "the import math is complex" - slice the cases and cover them.

## Definition of Done

Per item - `DONE` only when:

- [ ] The endpoint's test covers every branch (filters / validation errors / throttle)
- [ ] `just test --filter={TestClass}` passes - paste the result line
- [ ] The full suite is still green - paste the `Tests: N passed` line

Global: **G1** all three test classes exist and pass. **G2** `just test` fully green.

## Work List

| #   | Item                        | Status |
| --- | --------------------------- | ------ |
| 1   | ProductIndexTest            | `TODO` |
| 2   | ProductStoreTest            | `TODO` |
| 3   | ProductImportTest           | `TODO` |
