...
2. Is the work a **known finite list**, or must it be **discovered first**? If discovered - by what
   (grep / glob / `php artisan route:list` / a failing-test sweep)? Roughly how many items?
...
5. May it `git commit` / `push` / stage? May it open a GitHub PR/issue, or touch any external
   service? Any destructive ops (e.g. `just fresh` wipes the local db) or off-limits paths
   (committed migrations, `config/database.php`)?
...
- What's the before->after transformation, stated concretely enough to apply uniformly (e.g.
  inline controller queries -> repository methods, or model-scope extraction)?
- Isolation: do items touch shared files (-> serialize) or disjoint ones (-> can parallelize)?
- What's the per-item verification (`just test` / `just lint` / grep) that proves the change
  is safe?
...
- This repo has **no deployment target** - it runs locally only. A "deployment" goal here means
  local serving (`just start` on :8105) or authoring future deploy docs. If the developer wants a
  real deploy, first interrogate WHERE (host, method) - that decision is itself part of the goal.
- What's the health-verify that proves success (an HTTP 200 from `curl.exe` on :8105, a migration
  run, a seeded /api/products response)?
