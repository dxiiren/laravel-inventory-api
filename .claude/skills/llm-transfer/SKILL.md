...
## 1. Role
You are {persona - e.g. a senior Laravel + PHP 8 engineer / an Eloquent query-performance
specialist / a GraphQL schema reviewer}. {Any relevant seniority, domain, or mindset.}
...
situation: what the project is (a Laravel 12 product-inventory API: REST + GraphQL product
endpoints, queued Excel imports that adjust stock quantities), the domain, and WHY this task
matters. Self-contained - assume the reader has never seen this codebase.}
...
## 6. Constraints & Rules
- Tech stack: {Laravel 12 / PHP 8.4 / SQLite / Lighthouse GraphQL / maatwebsite Excel / PHPUnit 11, versions}
...
Target: _"Hand off a failing feature test to an external LLM for a fix."_

````text
# MASTER PROMPT - Fix upload validation rejecting valid 2 MB Excel files

## 1. Role
You are a senior Laravel engineer who writes precise, minimal fixes.

## 2. Mission
Make the Excel import endpoint accept a 2 MB .xlsx without breaking the 5 MB cap, and explain
the root cause in one paragraph.

## 3. Background & Context
Small Laravel 12 product-inventory API. Clients POST an .xlsx to /api/products/import; a
FormRequest validates the upload before a queued job imports the rows. Valid 2 MB files are
being rejected as too large. No repo access - everything you need is below.

## 4. Current State
- Done: files under ~1 MB import fine.
- Tried: re-exporting the spreadsheet - no effect.
- Failing: a 2 MB .xlsx returns a validation error "The Excel file must not exceed 5MB."

## 5. Relevant Artifacts
`app/Http/Requests/ImportProductRequest.php`
```
...
## 6. Constraints & Rules
- Laravel 12 validation rules. `max` on a file rule is in KILOBYTES. Keep the mimes rule untouched.

## 7. Your Task
Return the corrected rules() array and a one-paragraph root-cause explanation.

## 8. Output Format & Success Criteria
- Output: the fixed rules() only + the explanation.
- Done: files up to 5 MB (5120 KB) accepted, larger rejected, non-xlsx still rejected.

## 9. Open Questions & Assumptions
- Assume the form posts rating as a string ("1") - note if that matters.

## Reminder
Return the fixed rules() + a one-paragraph root cause. Keep the review rule untouched.
````
